...
(function ($) {
	...
	window.addEventListener("DOMContentLoaded", function () {
		...
		if (typeof elementorFrontend !== "undefined") {
			if (elementorFrontend.isEditMode()) {
				...
				isEditMode = true;
				// Check for iframe (editMode = load elements in iframe).
				if (window.frameElement) {
					const iframeContent = window.frameElement.contentDocument,
						elementorMain = iframeContent.querySelectorAll(".elementor");

					// When elementorMain is ajax loaded/changed.
					elementorMain[0].addEventListener("DOMSubtreeModified", function () {
						animatedElms =
							elementorMain[0].querySelectorAll(".mm-enter-animate");
	...
	var wcCategoriesMenu = function () {
		var wcCatMenuToggle = $(".micemade-elements_wc_cat_menu").find(
			".sub-toggler"
		);
	...
	$(window).on("elementor/frontend/init", function () {
		...
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/micemade-wc-products-tabs.default",
			micemadeElementsTabs
		);

		// Start Micemade Product Categories Menu
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/micemade-wc-cat-menu.default",
			wcCategoriesMenu
		);

		// Start Micemade Single product
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/micemade-wc-single-product.default",
			mmWcGalleryJS
		);
		...
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/micemade-wc-categories.default",
			function ($scope) {
			...
			function load_ajax_posts(e) {
				...
				if (
					...
					$.ajax({
						...
						beforeSend: function () {
							$loader
								.addClass("post_loading_loader")
								.html(micemadeJsLocalize.loadingposts);
						},
						success: function (data) {
							...
							if ($data.length) {
								...
								$loader
									...
									.html(micemadeJsLocalize.loadmore);
							} else {
								$loader
									.removeClass("post_loading_loader")
									.addClass("no_more_posts disabled")
									.html(micemadeJsLocalize.noposts);
							}
						},
